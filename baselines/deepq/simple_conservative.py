...
class ActWrapper(object):
    ...
    @staticmethod
    def load_with_scope_retrain_for_play(path, scope):
        with open(path, "rb") as f:
            model_data, act_params, _, _ = cloudpickle.load(f)
        act_params["scope"] = scope
        cur_graph = tf.Graph()
        with cur_graph.as_default():
            act = deepq.build_act(**act_params)
    ...
    @staticmethod
    def load_with_scope_for_retrain(path, scope):
        with open(path, "rb") as f:
            model_data, act_params, train, update_target = cloudpickle.load(f)
        act_params["scope"] = scope
        cur_graph = tf.Graph()
        with cur_graph.as_default():
            act = deepq.build_act(**act_params)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                zipfile.ZipFile(arc_path, 'r', zipfile.ZIP_DEFLATED).extractall(td)
                U.load_state(os.path.join(td, "model"))
            return (ActWrapper(act, act_params), train, update_target)
    ...
    def save_with_sess_for_retrain(self, sess, train, update_target, path=None):
        """Save model to a pickle located at `path`"""
        if path is None:
            path = os.path.join(logger.get_dir(), "model.pkl")
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(arc_name, "rb") as f:
                model_data = f.read()
        with open(path, "wb") as f:
            cloudpickle.dump((model_data, self._act_params, train, update_target), f)
...
def load_with_scope_retrain_for_play(path, scope):
    return ActWrapper.load_with_scope_retrain_for_play(path, scope)

def load_with_scope_for_retrain(path, scope):
    return ActWrapper.load_with_scope_for_retrain(path, scope)
...
def learn_and_save_for_retrain(env,
          q_func,
          lr=5e-4,
          max_timesteps=100000,
          buffer_size=50000,
    ...
    if path_for_save is not None:
        act.save_with_sess_for_retrain(sess, train, update_target, path=path_for_save)
    return act
...
def retrain_and_save(env,
    ...
    act, train, update_target, debug = load_with_scope_for_retrain(path_for_retrain, scope)
